...
process "qqPlot" {
    ...
    publishDir "${params.output}/positions/${model}", pattern: "${key}.DMRs*" , mode: 'copy', enabled: params.input ? true : false
    publishDir "${params.output}/regions/${model}", pattern: "${key}.region*" , mode: 'copy', enabled: params.input && params.DMRs ? true : false
