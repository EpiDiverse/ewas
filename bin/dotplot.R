...
gg.dotplot <- function(df, title, contigs){
    ...
    grids <- ifelse(chr.nrow < as.numeric(contigs), chr.nrow, as.numeric(contigs))
    ...
    p.html <- paste0(basename(title), ".html")
    p.zip <- paste0(title, ".zip")
    htmlwidgets::saveWidget(p, p.html, selfcontained=F, libdir=basename(title), title=basename(title))
    zip::zipr(p.zip, c(p.html,basename(title)))
